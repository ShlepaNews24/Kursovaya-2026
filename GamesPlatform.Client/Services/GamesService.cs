    ...
    public class GamesService : IGamesService
    {
        ...
        public async Task<bool> AddGameAsync(GameDto game)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(token)) { _notify.Show("Ошибка: вы не авторизованы", "error"); return false; }
        ...
        public async Task<bool> DeleteGameAsync(int id)
        {
            try
            {
                ...
                request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", token);
                var response = await _http.SendAsync(request);
                if (!response.IsSuccessStatusCode) { _notify.Show("Ошибка удаления игры", "error"); return false; }
        ...
        {
            try { return await _http.GetFromJsonAsync<RatingSummaryDto>($"api/ratings/game/{gameId}"); }
            catch (Exception ex) { _notify.Show($"Ошибка загрузки рейтинга: {ex.Message}", "error"); return null; }
        }
        ...
        public async Task<string?> UploadLogoAsync(Stream fileStream, string fileName)
        {
            try
            {
                ...
                using var request = new HttpRequestMessage(HttpMethod.Post, "api/games/upload-logo");
                request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", token);
        ...
        private class UploadLogoResponse
        {
